...
public class TimeSelectorA {
	...
	private String FORMAT_STR1 = "yyyy-MM-dd";
	...
	public TimeSelectorA(Context context, ResultHandler resultHandler,
		...
		startCalendar
				.setTime(AbDateUtil.getDateByFormat(startDate, FORMAT_STR));
		endCalendar.setTime(AbDateUtil.getDateByFormat(endDate, FORMAT_STR));
		initDialog();
		initView();
	}
	...
	private void initView() {
		...
		tv_select.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				handler.handle(AbDateUtil.getStringByFormat(
						selectedCalender.getTime(), FORMAT_STR1));
				seletorDialog.dismiss();
			}
		});
		...
		if (!TextUtils.isEmpty(workStart_str)
			...
			if (startTime.getTime().getTime() == endTime.getTime().getTime()
					|| (startWorkTime.getTime().getTime() < startTime.getTime()
							...
							.getTime().getTime())) {
				Toast.makeText(context, "Wrong parames!", Toast.LENGTH_LONG)
						.show();
				return false;
			}
			startCalendar
	...
	private void loadComponent() {
		...
		if (year1 == endYear) {
			...
			for (int i = 0; i < month1; i++) {
				if (i < 9) {
					month.add("0" + String.valueOf(i + 1));
				} else {
	...
	private void dayChange() {
		...
		} else if (month1 == endMonth && selectedMonth == month1) {
			day.clear();
			for (int i = 0; i < day1; i++) {
				if (i < 9) {
					day.add("0" + String.valueOf(i + 1));
	...
	private void minuteChange() {
		if ((scrollUnits & SCROLLTYPE.MINUTE.value) == SCROLLTYPE.MINUTE.value) {
			...
			selectedCalender.set(Calendar.MINUTE,
					Integer.parseInt(minute.get(0)));
			minute_pv.setData(minute);
			minute_pv.setSelected(0);
	...
	public void showCurrent() {
		...
		for (int i = 0; i <yearsize; i++) {
			if (Integer.parseInt(year.get(i), 10) == calendar.get(Calendar.YEAR)) {
		...
		for (int i = 0; i < monthsize; i++) {
			if (Integer.parseInt(month.get(i), 10) == calendar.get(Calendar.MONTH)+1) {
		...
		for (int i = 0; i <daysize; i++) {
			if (Integer.parseInt(day.get(i), 10) == calendar.get(Calendar.DAY_OF_MONTH)) {
	...
	public void  setDataFormat(String FORMAT_STR1){
		this.FORMAT_STR1=FORMAT_STR1;
	}
}
